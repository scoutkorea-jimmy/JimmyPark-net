...
(async () => {
  ...
  assert.equal(refreshed.version, 42);
  ...
  assert.equal(refreshed.pages.work.sections.video.cases.length, 13);
  ...
  assert.equal(backed.version, 42);
  ...
  assert.equal(stills.version, 42);
  ...
  assert.equal(filled.version, 42);
  ...
  assert.equal(bpFilled.version, 42);
  ...
  assert.deepEqual(await get(v37), defaults, 'The unchanged live v37 homepage must upgrade to the current personal-brand defaults');
  ...
  assert.deepEqual(await get(custom38), custom38, 'Homepage normalization must be idempotent');

  console.log('PASS: v2-v39 migrations, personal-brand homepage, BP Media card image, video stills, BP Media showcase, own-platform KDP, Scouting terminology, stale early seeds and Korean subtitle, website back-end details, showcase fields, +82 phone, Work → Media/Dev split with custom sections, visibility, order and links, retired project removal, additive evidence, empty edits, idempotence and no GET writes.');
