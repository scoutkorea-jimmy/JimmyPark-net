...
// H2: KV get/put failures fail closed (503) — never treat errors as fails=0.
export async function onRequestPost({ request, env }) {
  if (!env.TOTP_SECRET) return json({ ok: false, error: "not_configured" }, 503);
  if (!env.JP_KV) return json({ ok: false, error: "storage_unavailable" }, 503);
  let body = {};
  ...
  let fails = 0;
  try {
    fails = parseInt((await env.JP_KV.get(rlKey)) || "0", 10) || 0;
  } catch (_) {
  ...
  if (!ok) {
    try {
      await env.JP_KV.put(rlKey, String(fails + 1), { expirationTtl: 600 });
    } catch (_) {
      return json({ ok: false, error: "storage_unavailable" }, 503);
    }
