...
export async function onRequestGet({ env }) {
  const paths = ['', '/work', '/dev', '/lecture', '/scouting', '/contact', '/insights', '/sitemap'];
  let articles;
  ...
  articles.forEach(post => { urls.push('<url><loc>https://jimmypark.net/insights/' + encodeURIComponent(post.slug) + '</loc><lastmod>' + new Date(post.updatedAt).toISOString().slice(0, 10) + '</lastmod></url>'); });
